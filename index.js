...
// Function to handle automatic sending
function autoSendTalk() {
  if (!sessionInfo) {
    updateStatus(statusElement, 'Please create a connection first');
    return;
  }
  ...
  talkHandler(); // Call the existing talkHandler function
}

// Modify the taskInput event listener to reset the timeout every time the input changes
taskInput.addEventListener('input', () => {
  clearTimeout(inputTimeout); // Clear the existing timeout
  inputTimeout = setTimeout(autoSendTalk, 600); // Set a new timeout
});

// Existing talkHandler function (make sure it's reusable as is or with slight modifications if necessary)
...
async function enableWhisper() {
  if (!isDictating) {
    try {
      ...
      mediaRecorder.addEventListener("stop", async () => {
        ...
        if (data.text && !unwantedSentences.includes(data.text)) {
          ...
          updateStatus(statusElement, `Transcription: ${data.text}`);
          lastText = data.text;
      ...
      mediaRecorder.start();
      isDictating = true;
      updateStatus(statusElement, 'Dictation enabled');
      ...
      updateStatus(statusElement, 'Error enabling Whisper');
    }
  } else {
    ...
    isDictating = false;
    updateStatus(statusElement, 'Dictation disabled');
  }
}
...
document.querySelector('#whisperBtn').addEventListener('click', enableWhisper);
...
async function talkToOpenAI(prompt) {
  const response = await fetch(`/openai/complete`, { //http://localhost:3000
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
    },
